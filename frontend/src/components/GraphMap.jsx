...
export default function GraphMap({ wikiData, onSelectPage }) {
  ...
  const lockedRef = useRef(false); // ref instead of state — readable inside D3 closures
  const [lockedUI, setLockedUI] = useState(false); // mirrors lockedRef for UI only
    ...
    const node = svg.append("g")
      ...
      .call(d3.drag()
        ...
        .on("end",  (e, d) => {
          ...
          // Read from ref — always current, not a stale closure
          if (!lockedRef.current) { d.fx = null; d.fy = null; }
        })
      )
      .on("click", (e, d) => {
      ...
      .on("mouseover", function(e, d) {
        ...
        link
          ...
          .attr("stroke",         l => l.source.id === d.id || l.target.id === d.id ? "#FFD100" : "#1E2530");
      })
